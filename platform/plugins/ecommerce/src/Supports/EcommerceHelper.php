...
class EcommerceHelper
{
    ...
    protected array $availableDistricts = [];
    ...
    public function getAvailableDistricts(): array
    {
        if (! empty($this->availableDistricts)) {
            return $this->availableDistricts;
        }
    
        // Load districts from DB (id => name)
        ...
        // Fallback to ecommerce setting if no DB records
        try {
            $selectedDistricts = json_decode(get_ecommerce_setting('available_districts'), true);
        } catch (Exception) {
            $selectedDistricts = [];
        }
    
        $this->availableDistricts = $selectedDistricts ?? [];
